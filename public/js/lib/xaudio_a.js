...
// https://github.com/zhuomingliang/XAudioJS/blob/master/XAudioServer.js
...
XAudioServer.prototype.writeAudio = function (buffer) {
	// console.log(`XAudioServer: writeAudio:`, buffer);
	// if (this.audioType == 0) {
...
XAudioServer.prototype.initializeMozAudio = function () {
	console.log(`xaudio: initializeMozAudio`);
...
XAudioServer.prototype.initializeWebAudio = function () {
	...
	console.log(`xaudio: initializeWebAudio:`, launchedContext);
	if (launchedContext) {
...
XAudioServer.prototype.initializeFlashAudio = function () {
	console.log(`xaudio: initializeFlashAudio`);
...
XAudioServer.prototype.startAudio = function () {
	// audioContextHandle = new AudioContext();
	console.log(`startAudio:`, audioContextHandle);

	// var bufferSize = 4096;
	// var whiteNoise = audioContextHandle.createScriptProcessor(bufferSize, 1, 1);
	...
	// 	}
	// }
	// whiteNoise.connect(audioContextHandle.destination);
	...
	audioNode = audioContextHandle.createScriptProcessor(samplesPerCallback, 1, 2);			//Create 2 outputs and ignore the input buffer (Just copy buffer 1 over if mono)
	...
	// // audioSource.start(0);														//Start the loop!

	// var frequency = 880;
	// // var audioContextHandle = new AudioContext();
	// // Buffer size of 4096, 0 input channels, 1 output channel.
	// var scriptProcessorNode = audioContextHandle.createScriptProcessor(4096, 0, 1);
	// scriptProcessorNode.onaudioprocess = function (event) {
	// 	var startTime = audioContextHandle.currentTime;
	// 	var samples = event.outputBuffer.getChannelData(0);
	// 	for (var i = 0; i < 4096; i++) {
	// 		var t = startTime + (i / audioContextHandle.sampleRate);
	// 		// samples is a Float32Array
	// 		samples[i] = Math.sin(t * frequency);
	// 	}
	// };
	// // Route it to the main output.
	// scriptProcessorNode.connect(audioContextHandle.destination);
}
// XAudioServer.prototype.startAudio = function () {
// 	console.log(`XAudioServer: startAudio:`, audioContextHandle);
// 	// audioSource = audioContextHandle.createBufferSource();						//We need to create a false input to get the chain started.
// 	// audioSource.loop = false;	//Keep this alive forever (Event handler will know when to ouput.)
// 	// XAudioJSSampleRate = webAudioActualSampleRate = audioContextHandle.sampleRate;
// 	// audioSource.buffer = audioContextHandle.createBuffer(1, 1, webAudioActualSampleRate);	//Create a zero'd input buffer for the input to be valid.
...
function audioOutputEvent(event) {		//Web Audio API callback...
	...
	while (index < samplesPerCallback) {
		buffer2[index] = buffer1[index] = Math.random() * 0.25 - 0; //defaultNeutralValue;
		++index;
	}
}
...
// 		}
// 		catch (error) {
// 			console.log(`xaudio: get webkitAudioContext: error:`, error);
// 			try {
// 				audioContextHandle = new AudioContext();								//Create a system audio context.
// 			}
// 			catch (error) {
// 				console.log(`xaudio: get AudioContext: error:`, error);
// 				return;
// 			}
// 		}
// 		// try {
...
// 		// }
// 		// catch (error) {
// 		// 	console.log(`xaudio: get webkitAudioContext->audioSource: error:`, error);
// 		// 	return;
// 		// }
// 		launchedContext = true;
// 		console.log(`xaudio: launchedContext:`, launchedContext);
// 	}
// })();